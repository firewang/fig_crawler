...
def get_proxys(page=1):
    ...
    # 获取id为ip_list的table
    bf1_ip_list = BeautifulSoup(target_response.text, 'lxml')
    bf2_ip_list = BeautifulSoup(str(bf1_ip_list.find_all(id='ip_list')), 'lxml')
    ip_list_info = bf2_ip_list.table.contents
    # 存储代理的列表
    proxys_list = []
    # 爬取每个代理信息
    for index, cur_ip in enumerate(ip_list_info):
        if index % 2 == 1 and index != 1:
            dom = etree.HTML(str(cur_ip))
            ip = dom.xpath('//td[2]')
            port = dom.xpath('//td[3]')
...
def initpattern():
    """
    :return: lose_time - 匹配丢包数,waste_time - 匹配平均时间
    """
    # 匹配丢包数
...
def find_proxy():
    ...
    for page_index in range(1, 6):
        proxys_list = get_proxys(page_index)
        # 如果平均时间超过200ms重新选取ip
        while True:
